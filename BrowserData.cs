...
internal static class BrowserData
{
    ...
    public static string ItemName(int type)
    {
        try
        {
            ...
            return string.IsNullOrWhiteSpace(s) ? ("物品" + type) : s;
        }
        catch
        {
            return "物品" + type;
    ...
    public static string NpcName(int type)
    {
        try
        {
            ...
            return string.IsNullOrWhiteSpace(s) ? ("生物" + type) : s;
        }
        catch
        {
            return "生物" + type;
    ...
    private static string LeafSource(int itemType, bool allowLoot, bool allowMineable)
    {
        ...
        if (allowMineable && CanMine(itemType)) tags.Add("采集");
        if (allowLoot)
        {
            List<DropSource> drops = LootOf(itemType);
            if (drops.Count > 0)
            {
                tags.Add("掉落自 " + NpcName(drops[0].NpcType) + (drops.Count > 1 ? " 等" : ""));
            }
        }
