...
public static partial class Browser
{
    ...
    private static readonly string[] TabNames = { "Recipes", "Craft", "Items", "Bestiary" };
    ...
    // 搜索框按页各用各的，互不影响
    private static readonly SearchBox RecipeNameBox = new SearchBox("Search name", KeyIdName);
    private static readonly SearchBox RecipeDescBox = new SearchBox("Search tooltip", KeyIdDesc);
    private static readonly SearchBox ItemNameBox = new SearchBox("Search name", KeyIdItemName);
    private static readonly SearchBox ItemDescBox = new SearchBox("Search tooltip", KeyIdItemDesc);
    private static readonly SearchBox NpcNameBox = new SearchBox("Search name", KeyIdNpcName);
    ...
    private static void HistoryBack()
    {
        if (_historyPos <= 0)
        {
            VanillaTextTip("No query history yet");
            return;
        }
        _historyPos--;
    ...
    private static void HistoryForward()
    {
        ...
        {
            VanillaTextTip("End of query history");
            return;
        }
        _historyPos++;
        _queryType = _history[_historyPos];
    ...
    private static void DrawRecipeTab(int cx, int cy, int cw, int ch)
    {
        ...
        // 查询栏：原版 Left 16
        bool fromMouse;
        int q = DrawQuerySlot(new Rectangle(cx + 16, cy, Slot, Slot), _queryType, _queryFake, "Place item here", out fromMouse);
        if (q >= 0)
        {
            SetQuery(q);
        ...
        int gx = cx + 61;
        if (DrawRadioRow(gx, cy, _radio == 0, "All recipes", null))
        {
            _radio = 0;
            _gridDirty = true;
        }
        if (DrawRadioRow(gx, cy + RowH, _radio == 1, "Nearby chests", "Click to refresh"))
        {
            RefreshNearbyItems();
            _radio = 1;
            _gridDirty = true;
        }
        if (DrawRadioRow(gx, cy + RowH * 2, _tileChooser, "Crafting stations", null))
        {
    ...
    {
        bool real;
        int t = DrawQuerySlot(r, _craftRoot, true, "Place item here", out real);
        if (t < 0) return;
        _craftRoot = t;
        _craftDirty = true;
    }
    ...
    private static void DrawRecipeInfo(Rectangle bar)
    {
        ...
        AddCondition(names, okFlags, recipe.needWater, player != null && SafeAdj(player, 0), "Water");
        AddCondition(names, okFlags, recipe.needHoney, player != null && SafeAdj(player, 1), "Honey");
        AddCondition(names, okFlags, recipe.needLava, player != null && SafeAdj(player, 2), "Lava");
        AddCondition(names, okFlags, recipe.needSnowBiome, player != null && SafeZone(player, 0), "Snow biome");
        AddCondition(names, okFlags, recipe.needGraveyardBiome, player != null && SafeZone(player, 1), "Graveyard");
        ...
        if (total > textW && UIRenderer.IsMouseOver(x, y, textW, lh * 2))
            VanillaTextTip("Crafting station: " + string.Join(", ", names));
    ...
    private static void DrawCraftTab(int cx, int cy, int cw, int ch)
    {
        ...
        int tx = cx + 61;
        bool t1, t2, t3;
        tx += DrawToggle(tx, cy + 11, "Nested", _craftNested,
            "Recursively expand every ingredient tier\nTurn off to show only direct ingredients", out t1) + 10;
        tx += DrawToggle(tx, cy + 11, "Drops", _craftLoot,
            "Mark ingredients that only drop from enemies or containers\nAlso marks who drops them", out t2) + 10;
        DrawToggle(tx, cy + 11, "Gathered", _craftMine,
            "Mark ingredients you can gather directly from the world (ore, wood, etc.)", out t3);
        ...
        if (rows.Count == 0)
        {
            TextFit("No crafting recipes found for this item.", view.X + 4, view.Y + 8,
    ...
    private static void DrawCraftRow(Rectangle view, CraftNode node, int rowH, int y)
    {
        ...
        if (node.RecipeIndex >= 0)
        {
            ...
            string station = tile < 0 ? "By hand" : BrowserData.TileName(tile);
    ...
    private static void DrawItemTab(int cx, int cy, int cw, int ch)
    {
        ...
        if (DrawCheckRow(cx + cw - 270, cy, _itemCrafted, "Craftable", "Show only craftable items"))
        {
            _itemCrafted = !_itemCrafted;
            _gridDirty = true;
        }
        if (DrawCheckRow(cx + cw - 270, cy + 30, _itemLoot, "Drops", "Show only dropped items"))
        {
            _itemLoot = !_itemLoot;
            _gridDirty = true;
        }
    ...
    private static void DrawBestiaryTab(int cx, int cy, int cw, int ch)
    {
        ...
        bool fromMouse;
        int q = DrawQuerySlot(new Rectangle(cx, cy, Slot, Slot), _npcQueryType, _npcQueryFake, "Place item here", out fromMouse);
        if (q >= 0)
        {
            _npcQueryType = q;
        ...
        int gx = cx + 45;
        if (DrawRadioRow(gx, cy, _npcSort == 0, BestiarySortLabel("Sort_BestiaryID", "Bestiary ID"), null))
        {
            _npcSort = 0;
            _gridDirty = true;
        }
        ...
        string encTip = _npcUnencountered
            ? "Show only undefeated enemies\n(right-click to switch to defeated enemies)"
            : "Show only defeated enemies\n(right-click to switch to undefeated enemies)";
        if (DrawCheckRow(chkX, cy + ch - RowH - 30, _npcEncountered,
                _npcUnencountered ? "Not encountered" : "Encountered", encTip, out right))
        {
        ...
        if (DrawCheckRow(chkX, cy + ch - RowH, _npcHasLoot, "Has drops", "Show only enemies that have drops"))
        {
